...
/**
 * CssColor.make("rgba(8,180,75,1)")
 * 
 * @author prayag
 * 
 */

public class Landscape implements IRenderComponent, IUpdateComponent {
	...
	public void generatePosition(int position) {
		...
		topHeight[position] = Math.min(
				Math.max(prevTop + Random.nextInt(20) * shrink, 0), height
						- minGap);
		bottomHeight[position] = Math.min(
				Math.max(prevBottom + Random.nextInt(20) * shrink, 0), height
						- minGap);
	...
	public void render(Context2d context) {
		...
		for (int i = 0; i < topHeight.length; ++i) {
			...
			if (bestPos < 0 && isPosBestScore(i + slideI)) {
				...
				context.fillRect(i * SEGMENT_WIDTH - slideF - 1, 0,
						SEGMENT_WIDTH + 2, topHeight[pos]);
				context.fillRect(i * SEGMENT_WIDTH - slideF - 1, height
						- bottomHeight[pos], SEGMENT_WIDTH + 2,
						bottomHeight[pos]);
				if (centerHeight[pos] > 0) {
					int gap = (height - bottomHeight[pos] - topHeight[pos] - centerHeight[pos]) / 2;
					context.fillRect(i * SEGMENT_WIDTH - slideF - 1,
							topHeight[pos] + gap, SEGMENT_WIDTH + 2,
							centerHeight[pos]);
				}
		...
		if (bestPos >= 0) {
			...
			context.fillRect(bestI * SEGMENT_WIDTH - slideF - 1, 0,
					SEGMENT_WIDTH + 2, topHeight[bestPos]);
			context.fillRect(bestI * SEGMENT_WIDTH - slideF - 1, height
					- bottomHeight[bestPos], SEGMENT_WIDTH + 2,
					bottomHeight[bestPos]);
			context.fill();
		}
	}
